var TurnGame = (function () {
    ...
    var initiate = function (heroName) {
        ...
        return{
            ...
            toggleMenu: function(){ //style.display = none 일때 감춰지고 block 일때 안보여진다.
            ...
            generateMonster: function(){
                ...
                return this.toggleMenu(); //지금 드는 궁금증은.. toglemenu에서 공격메뉴, 기본메뉴의 보여짐이다.
            },
            ...
            nextTurn: function(){   //이부분 로직 확인
                var self = this;
                turn = !turn;
                document.getElementById('battle-button').disabled = true;//몬스터공격시 버튼사용불가
                if(!turn){
                    window.setTimeout(function(){
            ...
            win:function(){
                this.setMessage(monster.name+' 사냥에 성공해 경험치 '+monster.xp+'를 얻었습니다.');
                hero.xp += monster.xp;
            ...
            gameOver: function(){
                document.getElementById('screen').innerHTML = hero.name+'은 레벨'+hero.lev+'에서 죽었습니다. 새로 시작하려면 F5키를 눌러주세요!';
                return false;
            },
            exit: function(){
                document.getElementById('screen').innerHTML = '이용해주셔서 감사합니다. 새로시작하려면 F5를 눌러주세요';
            },
        };
    };
    return {
...
document.getElementById('start-screen').onsubmit = function(e){
    var name = document.getElementById('name-input').value; //value === nodevalue 인지 확인해보기
    e.preventDefault();
